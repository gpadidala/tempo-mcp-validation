...
Against Tempo 2.10.7 on a clean stack:

- **37/37** pytest checks pass (protocol, contract, search, trace-by-id, tags,
  metrics, parity, negative, multi-tenancy, security).
- **9/9** use cases pass — `reports/usecases.md` (generated by `make usecases`;
  published as a CI artifact and in the job summary).
- **7 tools** discovered at runtime, zero drift vs `tools_snapshot.json`.
- Validation matrix is generated from the live server:
